...
def loss_function(recon_x, x, mu, logvar) -> Variable:
    ...
    #  -D_{KL} = 0.5 * sum(1 + log(var) - mu^2 - var)
    #   D_{KL} = -0.5 * sum(1 + logvar - mu^2 - exp(logvar))
...
def train(epoch):
    ...
    for batch_idx, (data, _) in enumerate(train_loader):
        ...
        loss, batch_bce, batch_kl = loss_function(recon_batch, data, mu, logvar)
        bce += batch_bce.item()
        kl += batch_kl.item()
        ...
        if batch_idx % LOG_INTERVAL == 0:
            print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f} bce: {} kl: {}'.format(
                epoch, batch_idx * len(data), len(train_loader.dataset),
                100. * batch_idx / len(train_loader),
                loss.item() / len(data), batch_bce / len(data), batch_kl / len(data)))
